...
final class LilaFuture[A](private val fua: Fu[A]) extends AnyVal {
  ...
  def effectFold(fail: Exception => Unit, succ: A => Unit)(implicit
      ec: EC
  ): Unit = {
    fua onComplete {
  ...
  def flatFold[B](fail: Exception => Fu[B], succ: A => Fu[B])(implicit
      ec: EC
  ): Fu[B] =
    fua flatMap succ recoverWith { case e: Exception => fail(e) }

  // def logFailure(logger: => lila.log.Logger, msg: Throwable => String)(implicit
  //     ec: EC
  // ): Fu[A] =
  //   addFailureEffect { e =>
  //     logger.warn(msg(e), e)
  ...
  def addEffects(fail: Exception => Unit, succ: A => Unit)(implicit
      ec: EC
  ): Fu[A] = {
    fua onComplete {
      case scala.util.Failure(e: Exception) => fail(e)
  ...
  // def monTry(path: Try[A] => lila.mon.TimerPath) =
  ...
  //   chronometer.monValue(path).result

  // def logTime(name: String) = chronometer pp name
  // def logTimeIfGt(name: String, duration: FiniteDuration) =
  //   chronometer.ppIfGt(name, duration)

  // def recoverDefault(implicit z: Zero[A], ec: EC): Fu[A] = recoverDefault(
  //   z.zero
  // )
...
final class LilaFutureOption[A](private val fua: Fu[Option[A]]) extends AnyVal {
  ...
  def getOrElse(other: => Fu[A])(implicit ec: EC): Fu[A] = fua flatMap {
...
final class LilaIterableFuture[A, M[X] <: IterableOnce[X]](
    private val t: M[Fu[A]]
) extends AnyVal {
  def sequenceFu(implicit bf: BuildFrom[M[Fu[A]], A, M[A]], ec: EC): Fu[M[A]] =
    Future.sequence(t)
}
